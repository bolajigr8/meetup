...
export const POST = withErrorHandler(async (req) => {
  ...
  // BUG FIX: populate assignedTo before responding — every other meeting
  // endpoint (GET list, PATCH) already does this. Without it, the
  // response's assignedTo field was raw ID strings instead of full user
  // objects, which crashed the Flutter app's parser on every create,
  // even though the meeting itself saved successfully.
  await meeting.populate('assignedTo', 'name email image')
